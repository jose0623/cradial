                                    ...
                                    @foreach ($reportes as $reporte)
                                        <tr>
                                            ...
                                            <td class="" style="text-align: right;">
                                                <a class="btn btn-sm btn-info " href="http://cradial.test/emisoras/1/programas"><i class="fa fa-play"></i>  Cargar</a>
                                            </td>
                                            <td>
                                                <form action="{{ route('reportes.destroy', $reporte->id) }}" method="POST">
                                                    <a class="btn btn-sm btn-primary " href="{{ route('reportes.show', $reporte->id) }}"><i class="fa fa-fw fa-eye"></i> {{ __('Show') }}</a>
                                                    <a class="btn btn-sm btn-success" href="{{ route('reportes.edit', $reporte->id) }}"><i class="fa fa-fw fa-edit"></i> {{ __('Edit') }}</a>
                                                    @csrf
                                                    @method('DELETE')
